...
!--------------------------------------------------------------------    
   if(iemis2) then 
    ...
    if (inc_emis) then
      ...
      call read_chimere_emis_cdf( femis2(ific), nx2, ny2, nsnap1, 1, 1, nvar_emis, var_emis_name, tbl_ratio_emi_snap , emis_g2(:,:,:), idebug  )
      end do
    else
...
! Lit les emissions de la SNAP 2, 4 et 7
   if (iemis1) then 
    ...
    do ific=1, nfile_emis
    call read_chimere_emis_cdf( femis1(ific), nx0, ny0, nsnap1, dx1min, dy1min, nvar_emis, var_emis_name, tbl_ratio_emi_snap, emis_g0(:,:,:), idebug  )
    end do
    do ivar=1, nvar_emis
...
! calcule les valeurs de topo_g0 a partir de topo_g2
  ...
  if (iground1.and.idebug)write(*,*) 'Lit lon,lat,topo,landuse COARSE dans '//trim(fground2)	
  if(iground1)call read_chimere_ground_cdf(fground1, nx1, ny1, lon_g0, lat_g0, landuse_g0, topo_g0, idebug )                       	                        
!--------------------------------------------------------------------        
...
!--------------------------------------------------------------------            
   do it=1,numFrTimes
      ...
      !--------------------------------------------------------------------
       ...
       call interpolation_donnees_grille
      !---------------------------------------------------------------
      ! Recalcul des variables
      !---------------------------------------------------------------   
      ...
      !---------------------------------------------------------------
      if (idebug)write(*,*) 'Calcul des profils moyen des variables...'      
	...
	if (met_itemp.ne.0) then 
	 if (icorrect_z_var(met_itemp)) then
      ...
      !---------------------------------------------------------------       
      ! Calcul vent 10 mètres
      !---------------------------------------------------------------           
      if (iwin) then      
        write(*,*) '>>>>>>>>>>>> Calcul du vent 10 m...'
