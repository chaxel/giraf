...
!--------------------------------------------------------------------        
      subroutine read_chimere_emis_cdf(femis, nx, ny, nsnap1, dxmin, dymin, nvar_emis, var_emis_name, tbl_ratio_emi_snap,  emis, idebug )
...
! fichiers d'emissions SNAP 3 
	...
	allocate( tab_code_snap(ncode_snap1))    
	 ...
	 start3 = (/dxmin,dymin, ns/)
         count3 = (/   nx,   ny, 1 /)       
         do ivar= 1, nvar_emis   	    
           call check(nf90_inq_varid(ncFileID, trim(var_emis_name(ivar)), xVarID))	 
	...
	! aditionne les emissions         
	 emis = emis + emis_tmp * tbl_ratio_emi_snap(snap1)      
        ...
        if (idebug)write(*,*) '<<<<<<<<<<<< ATTENTION : pas d emissions disponibles dans '//trim(femis)//'>>>>>>>>>>'   	  
      end if
