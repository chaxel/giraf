...
! Grillle de  sorties
  ...
  !Define timeString
    ...
    if (iwin) then    
      call check(nf90_def_var(ncFileID,'w10m',nf90_float,dimids=count3d,varID=XVarID))    
...
!--------------------------------------------------------------------
	...
	if (nf90_inq_varid(ncFileID, trim(var_name(ivar)), xVarID).eq.nf90_noerr) then
      ...
      ! vitesse 10 m (version 1.1)
      if ( iwin ) then
	if (nf90_inq_varid(ncFileID, 'w10m', xVarID).eq.nf90_noerr) then
          if (idebug)write(*,*) 'w10m'
