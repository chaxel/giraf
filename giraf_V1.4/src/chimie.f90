...
!--------------------------------------------------------------------        
      subroutine read_chimere_emis_cdf(femis, it, nx, ny, nsnap1, dxmin, dymin, nvar_emis, var_emis_name, ivar_emis, tbl_ratio_emi_snap, emis, idebug )
...
! fichiers d'emissions SNAP 3 
	...
	stop 1
	  ...
	  if(idebug)write(*,*)'Trouve emissions de PM10 dans '//trim(var_emis_name(ivar))
          emis(emis_ipm10,:,:) = emis(emis_ipm10,:,:) + emis(ivar,:,:)
	  ...
	  if(idebug)write(*,*)'Trouve emissions de PM25 dans '//trim(var_emis_name(ivar))
          emis(emis_ipm25,:,:) = emis(emis_ipm25,:,:) + emis(ivar,:,:)
          ivar_emis(emis_ipm25) = .true.
	end if
      end do      
      
      ! TRAITEMENT DES EMISSIONS NOX=NO+NO2
      if(idebug)write(*,*)'Utilise emissions de NOX dans '//trim(var_emis_name(emis_ino2))
      if (maxval(emis(emis_ino,:,:))+maxval(emis(emis_ino2,:,:)).gt.0.) then      
        if(idebug)write(*,*)'NOX = NO + NO2'
        ivar_emis(emis_inox) = .true.	
        emis(emis_inox,:,:) = emis(emis_ino,:,:) + emis(emis_ino2,:,:)
      else
        if(idebug)write(*,*)'NO  = 0.92 * NOX'
	if(idebug)write(*,*)'NO2 = 0.08 * NOX'
        emis(emis_ino ,:,:) = emis(emis_inox,:,:) * .92
	emis(emis_ino2,:,:) = emis(emis_inox,:,:) * .08
      end if 


      if (idebug) then
        do ivar= 1, nvar_emis      
          if (iemis_1h)write(*,*) 'Emissions HORAIRES max.domaine '//trim(var_emis_name(ivar))//'(molec/h/km2)=',maxval(emis(ivar,:,:))
          if (.not.iemis_1h)write(*,*) 'Emissions ANNUELLES max.domaine '//trim(var_emis_name(ivar))//'(kg/an/km2)=',maxval(emis(ivar,:,:))
	end do 
      end if          
...
! Corrections      
      ...
      if (correct_no) then
	    ...
	    if ( no_g1(i,j,1) .gt. no_g1(i,j,2) ) then ! dans panache ? -> NON car niveau 1 > niveau 2             	     
	     ...
	     dno_loc = ( no_g1(i,j,1) - no_g1(i,j,2) ) * ratio_emis_no(i,j)
	    else
	     dno_loc = 0.
	    end if
            ! corrige le NO
	    ...
	    if ( emis_no(i,j) .gt. 0) then
	      no2_g2(i,j,1) = no2_g2(i,j,1) + dno_loc * (emis_no2(i,j)/massmol_no2) / (emis_no(i,j)/massmol_no2)
	    !else
