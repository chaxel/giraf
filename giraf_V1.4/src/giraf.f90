   ...
   if(iemis2) then 
    ...
    if (inc_emis) then
      ...
      do ific=1, nfile_emis
        it = 1
        call read_chimere_emis_cdf( femis2(ific), it, nx2, ny2, nsnap1, 1, 1, nvar_emis, var_emis_name, ivar_emis, tbl_ratio_emi_snap , emis_g2(:,:,:), idebug  )
      end do
    else
      ...
      call read_x_y_ncol_ascii( femis2(1), nvar_emis, nx2, ny2, lon_g2(:,:),lat_g2(:,:),emis_g2(:,:,1:2), idebug )
      emis_inox  = 0
      emis_ipm10 = 0
...
! Lit les emissions de la SNAP 2, 4 et 7
   if (iemis1) then 
    ...
    do ific=1, nfile_emis
    call read_chimere_emis_cdf( femis1(ific), it, nx0, ny0, nsnap1, dx1min, dy1min, nvar_emis, var_emis_name, ivar_emis, tbl_ratio_emi_snap, emis_g0(:,:,:), idebug  )
    end do
    do ivar=1, nvar_emis
...
!--------------------------------------------------------------------            
   do it=1,numFrTimes
      !-------------------------------------------------------------------- 
      ! Lit les emissions horaires ou annuelles
      !-------------------------------------------------------------------- 
      if(iraf_chim) then 
        emis_g2 = 0.
        if (idebug)write(*,*) 'Lit les emissions NetCDF du domaine FINE...'    
