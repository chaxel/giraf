...
! Grillle de  sorties
  ...
  !Define timeString  
    ...
    do ivar = 1, nvar_emis 
      ...
      if (ivar_emis(ivar)) then
        call check(nf90_def_var(ncFileID,'emis_'//trim(var_emis_name(ivar)),nf90_float,dimids=count2d,varID=XVarID))    
...
!    call check(nf90_put_att(ncFileID, XVarID,  "_FillValue", -9999.0 ) )     
    ...
    ! Global attributes
    call check(nf90_put_att(ncFileID, nf90_global, 'title'  , 'Raffinement de maillage v'//trim(adjustl(version_str))//' - (c) AtmoRA 2010' ))      
...
!--------------------------------------------------------------------
	...
	if (nf90_inq_varid(ncFileID, 'emis_'//trim(var_emis_name(ivar)), xVarID).eq.nf90_noerr) then
           ...
           call check(nf90_put_var(ncFileID, XVarID, emis_g2(ivar,:,:), start = (/ 1,1/) ))		     
	end if       	
      end do
           	      
      call check(nf90_close(ncFileID))  
            
      end subroutine write_emis_an_cdf
!--------------------------------------------------------------------
