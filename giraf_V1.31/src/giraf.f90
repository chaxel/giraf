...
!*				  date: mars 2010                   *
!*          auteur: E. Chaxel (Atmo Rhone-Alpes)                    *
!*          email:  echaxel@atmo-rhonealpes.org                     *
!********************************************************************
...
!--------------------------------------------------------------------    
   if(iemis2) then 
    ...
    if (inc_emis) then
      ...
      call read_chimere_emis_cdf( femis2(ific), nx2, ny2, nsnap1, 1, 1, nvar_emis, var_emis_name, ivar_emis, tbl_ratio_emi_snap , emis_g2(:,:,:), idebug  )
      end do
    else
      ! en ASCII, plus simple un seul fichier     
      if (idebug)write(*,*) 'Lit les emissions ASCII du domaine FINE...'        
      call read_x_y_ncol_ascii( femis2(1), nvar_emis, nx2, ny2, lon_g2(:,:),lat_g2(:,:),emis_g2(:,:,:), idebug )
    end if   
    do ivar=1, nvar_emis
      if (idebug)write(*,*) 'Domaine FINE emissions max.(kg/an)'//trim(var_emis_name(ivar)),maxval(emis_g2(ivar,:,:))
    end do    
   else
      iraf_chim = .false.  
...
! Lit les emissions de la SNAP 2, 4 et 7
   if (iemis1) then 
    ...
    do ific=1, nfile_emis
    call read_chimere_emis_cdf( femis1(ific), nx0, ny0, nsnap1, dx1min, dy1min, nvar_emis, var_emis_name, ivar_emis, tbl_ratio_emi_snap, emis_g0(:,:,:), idebug  )
    end do
    do ivar=1, nvar_emis
...
!--------------------------------------------------------------------         
   ...
   do ivar=1, nvar_emis
    ...
    if (trim(var_emis_name(ivar)).eq.'PPM_coa' )emis_ippm_coa  = ivar    
    if (trim(var_emis_name(ivar)).eq.'PPM_fin' )emis_ippm_fin  = ivar        
   end do
...
!-------------------------------------------------------------------- 
   if (idebug)write(*,*) 'Ecrit emissions annuelles dans fichier sortie...'	 
   call write_emis_an_cdf
...
!--------------------------------------------------------------------            
   do it=1,numFrTimes
      ...
      call rafinement_gaz(nx1,ny1,nz1,ratio_emis_g2(emis_inox,:,:), &
      ...
      !si PPM_fin et PPM_coa sont disponibles dans les emissions sinon PM10
      if(ivar_emis(emis_ippm_fin).and.ivar_emis(emis_ippm_fin))&
         ratio_emis_g2(emis_ipm10,:,:) = &
	   (ratio_emis_g2(emis_ippm_fin,:,:) + ratio_emis_g2(emis_ippm_coa,:,:))/2
