subroutine wcal2d(nx_coa,ny_coa,lon_coa,lat_coa,nx_fin,ny_fin,lon_fin,lat_fin,ix,iy,wx,wy,proj) 
...
!Projection des entrées
  ...
  do i=1,nx_coa
    ...
    !write(*,*) gs1,x2,y2,h1,gs2,x2,y2,h2       !write(*,*) gs2,x2,y2,x3,y3,zone,proj2,div          
      x1 = lon_coa(i,j) * d2r
      y1 = lat_coa(i,j) * d2r
  ...
  do i=1,nx_fin
    do j=1,ny_fin
      ...
      do nx=1,nx_coa-1 
        ...
        enddo
      enddo
      write(*,*)lon_fin(i,j),lat_fin(i,j)
      write(*,'("* POINT : ",2I5,2F14.3,2F10.5," OUTSIDE DOMAIN (nsec=",I2,")")')i,j,x,y,lon_fin(i,j),lat_fin(i,j),nsec
      outside = outside + 1
!      print *,'nsec=',nsec
...
1111 continue 
  ...
  write(*,*) 'Point en dehors du domaine:', outside
