...
!----------------------------------------------------------------------------------------------------------------
	...
	emis_g2(emis_ino2,:,:) = emis_g2(emis_inox,:,:) * taux_no2_nox
      ...
      if (idebug) then
        do ivar= 1, nvar_emis      
          if (iemis_1h)write(*,*) 'Emissions HORAIRES max.domaine '//trim(var_emis_name(ivar))//'(molec/h/km2)=',maxval(emis_g2(ivar,:,:))
          if (.not.iemis_1h)write(*,*) 'Emissions ANNUELLES max.domaine '//trim(var_emis_name(ivar))//'(kg/an/km2)=',maxval(emis_g2(ivar,:,:))
	end do 
      end if  
