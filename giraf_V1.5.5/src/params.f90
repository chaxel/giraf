      ...
      integer, parameter    :: nvar = 11   ! utilise 3 polluants gaz O3, NO, NO2                  PMs :  PM10  pPPM  p10HNO3
      character(len=fnstrl) :: var_name(nvar)        = (/ 'O3' ,    'NO',  'NO2', 'C6H6',  'SO2', 'PM10',  'PM25',  'pPPM', 'pHNO3',  'temp',  'wins'  /)
      character(len=fnstrl) :: var_unit(nvar)        = (/ 'ppb',   'ppb',  'ppb',  'ppb',  'ppb','ug/m3', 'ug/m3', 'ug/m3', 'ug/m3',  'degC',	'm/s'  /)     
      character(len=fnstrl) :: var_name_cdf(nvar)   
      integer, parameter    :: voisinage_xy_var(nvar)= (/    10,      10,     10,     10,     10,     10,      10,	0,	10,	  10,	   10  /) ! taille des demi-voisinages (1 km)
 
      logical               :: iinput_var(nvar)      ! la variable est disponiblesponiblesponible le NetC? 
      logical               :: icorrect_z_var(nvar)  = (/ .true., .true., .true., .true., .true., .true.,  .true., .false.,  .true.,  .true.,	.true. /) ! corrige O3/NO2/PM10 avec profils Z ?    
      logical               :: ioutput_var(nvar)     = (/ .true., .true., .true., .true., .true., .true.,  .true., .false.,  .true.,  .true.,	.true. /) ! ecrit la variables dans NetCDF ?
       ! DEBUG
